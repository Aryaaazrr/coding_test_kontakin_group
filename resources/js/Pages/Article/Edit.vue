...
const article = page.props.article; // Data artikel untuk diedit
const currentUser = page.props.auth.user;

const fileError = ref("");

// Schema validasi, file sekarang opsional (boleh tidak ganti file)
const formSchema = toTypedSchema(
    z.object({
        ...
        status_konfirmasi: z.enum(["disetujui", "revisi"]).optional(),
        file: z
            .any()
            .optional()
            .refine((files) => {
                if (!files) return true; // file boleh kosong (tidak ganti file)
                if (files.length !== 1) return false;
                if (files[0].type !== "application/pdf") return false;
                const sizeKB = files[0].size / 1024;
                return sizeKB >= 100 && sizeKB <= 500;
            }, "File harus berupa PDF dengan ukuran 100 KB sampai 500 KB"),
    })
);
...
const { handleSubmit, values, resetForm, setFieldValue, errors } = useVeeForm({
    ...
    initialValues: {
        id_mahasiswa: article.id_mahasiswa,
        id_dosen: article.id_dosen,
        judul: article.judul,
        tipe: article.tipe,
        file: null,
    },
...
const onSubmit = handleSubmit((formValues) => {
    ...
    // Kalau user upload file baru, masukkan ke form data
    if (formValues.file && formValues.file.length === 1) {
        formData.append("file", formValues.file[0]);
    }

    // Kirim PUT/PATCH request ke endpoint update article
    router.post(`/article/${article.id}`, formData, {
            ...
            <div class="p-4">
                <div class="bg-white border rounded-xl p-6 max-w-full mx-auto">
                    <div class="flex flex-col items-start mb-4">
                        ...
                        <p>Form ini digunakan untuk mengubah data artikel</p>
                    </div>
                    ...
                    >
                        ...
                        <FormField name="file">
                            <FormItem>
                                ...
                                <p
                                    v-if="article.file_name"
                                    class="text-sm mt-1 text-gray-500"
                                >
                                    File saat ini:
                                    <strong>{{ article.file_name }}</strong>
                                </p>
                                <FormMessage />
                                <p
                                    v-if="fileError"
                        ...
                        <FormField v-if="currentUser.role === 'administrator'">
                            name="status_konfirmasi" v-slot="{ componentField }"
                            >
                            <FormItem>
